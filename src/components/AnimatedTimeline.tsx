...
const AnimatedTimeline = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 bg-gradient-to-r from-sky-400 to-sky-600 bg-clip-text text-transparent">Your Journey With Us</h2>
      
      <div className="relative">
        {/* Vertical line */}
        <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-sky-300 to-sky-500 transform md:-translate-x-1/2"></div>
        ...
        {steps.map((step, index) => {
          ...
          return (
            ...
            >
              {/* Circle on timeline */}
              <div className="absolute left-4 md:left-1/2 w-8 h-8 rounded-full bg-white border-4 border-sky-500 transform -translate-x-4 md:-translate-x-1/2 z-10"></div>
              
              {/* Content */}
              <div className={`w-full md:w-5/12 pl-12 md:pl-0 ${isEven ? 'md:pr-16' : 'md:pl-16'}`}>
                <div className="p-6 rounded-lg bg-gray-700 border border-sky-500 shadow-xl hover:shadow-2xl transition-shadow duration-300 transform hover:-translate-y-1">
                  <div className="flex items-center mb-3">
                    <div className="bg-gradient-to-r from-sky-400 to-sky-600 text-white text-xl font-bold rounded-full w-10 h-10 flex items-center justify-center mr-3">
                      {step.number}
                    </div>
                    <h3 className="text-xl font-bold text-sky-600">{step.title}</h3>
                  </div>
